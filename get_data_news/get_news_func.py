...
async def get_news_ixbt():
    ...
    for news in all_data:
        ...
        all_pretty_data.append({"title": title, "url": url, "date": time, "tags": tags})
    ...
    result_data = list(
        filter(lambda x: need_format_data(x["date"]) > date_last_news, all_pretty_data)
    )

    date_last_news = max(
        [dt.strptime(time, "%H:%M") for time in [x["date"] for x in all_pretty_data]]
    ).strftime("%H:%M")
